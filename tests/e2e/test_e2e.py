...
def split_output_uri(output_uri: str) -> tuple[str, str]:
    """Splits an output uri into its bucket name and blob path.

    Args:
        output_uri: (str) The output uri to split
...
def test_gcs_uri(output_uri: str, storage_client: storage.Client) -> Optional[int]:
...
def test_vertex_model_uri(
    ...
    Returns:
        bool: True if the model exists in Vertex AI, otherwise False.
    """
...
def pipeline_e2e_test(
    ...
    **kwargs
    ...
    try:
        pl = trigger_pipeline_from_payload(payload, use_fallback=False)
        pl.wait()
